...
@app.route('/api/resultss', methods=["POST"])
def index():
    if request.method == 'POST':
        ...
        if 'tic_id' in data:
            ...
            try:
                ...
                flux = lc.flux
                median_flux = np.nanmedian(flux).value  # Extract the numeric value using .value
                ...
                response_data = {
                    'tic_id': tic_id,
                    'amplitude': float(median_flux),
                    'num_trees': float(tree_normalized),
            ...
            return jsonify({'error': 'Missing tic_id in the request body'}), 400
